...
public class AmdocsTesting {
    ...
    public void testFirstRule() throws IOException {
        ...
        correctAlicuota1 = "1,20160801,08,N,2229,02386471,CB222902386471,DNI,18831752,CAICHEO NANCY SOLANGE,-.01,-.01,.00,0,.00,.00,.00,.00,.00,.00,.00,.00,F,PES,0001000000,1,,66317370727505,20160811,-,47366138|47366138";
        assertEquals(comprobante.getAlicuota(0),correctAlicuota1);
        comprobante.getAlicuotas().clear();
        ...
        amdocsFileAnalyser.initSecondRule(comprobante);
        correctAlicuota1 = "1,20160801,08,N,2195,00001881,CB219500001881,DNI,12367679,IFRAN ROBERTO,-44.21,-.01,-40.00,10.5,-4.20,.00,.00,.00,.00,.00,.00,.00,F,PES,0001000000,1,,66337578406012,20160822,-,68178525|501679215";
        assertEquals(comprobante.getAlicuota(0),correctAlicuota1);
        assertEquals(comprobante.getAlicuotas().size(),1);
        comprobante.getAlicuotas().clear();
    }
